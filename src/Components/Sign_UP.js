...
const Sign_UP = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (user) {
        ...
        const response = await axios.post("http://13.235.71.191:3000/api/users/save", {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post("http://13.235.71.191:3000/api/auth/users/register", input);
