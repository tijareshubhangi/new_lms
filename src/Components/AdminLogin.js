...
const AdminLogin = () => {
  ...
  const sendOtp = async () => {
    ...
    try {
      const response = await axios.post('http://13.201.186.255:9000/send-otp', { email });
  ...
  const verifyOtp = async () => {
    try {
      const response = await axios.post('http://13.201.186.255:9000/verify-otp', { email, userOtp: otp });
      setMessage(response.data.message);
